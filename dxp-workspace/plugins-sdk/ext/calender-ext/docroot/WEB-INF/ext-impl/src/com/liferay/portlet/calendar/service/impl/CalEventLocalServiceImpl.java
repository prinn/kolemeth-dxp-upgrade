...
public class CalEventLocalServiceImpl extends CalEventLocalServiceBaseImpl {
	...
	protected void importICal4j(long userId, long groupId, VEvent event)
		...
		if(Validator.isNotNull(event.getStartDate())  && 
				event.getStartDate().toString().indexOf("DTSTART;") == 0){
		 ...
		 * HebCal ics file will be imported into Liferay. These events should be categorized as Holiday
		 */
		...
		assetVocabularies = (List<AssetVocabulary>) AssetVocabularyLocalServiceUtil.dynamicQuery(dynamicQueryForVocabulary);
			...
			if (assetVocabularyCalendar != null) {
				...
				for(AssetCategory assetCategory : assetCategories){
					...
					if(assetCategory.getName().equalsIgnoreCase("holiday")){
		...
		int durationHour = (int)Math.abs(durationHours);
		int durationMinute = (int)Math.abs(durationMins);
