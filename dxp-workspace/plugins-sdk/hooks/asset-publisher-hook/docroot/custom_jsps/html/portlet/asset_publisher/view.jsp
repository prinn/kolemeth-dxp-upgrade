...
<c:if test="<%= enableKolEmethView && Validator.isNotNull(upcomingType) && newtotal > 0 %>">
	<%
	...
	if (assetVocabularyCalendar != null) {
		...
		for (AssetCategory assetCategory : assetCategorieslist) {
			...
			if(categoryType.equalsIgnoreCase(assetCategory.getTitle(themeDisplay.getLocale()))){
					...
					if (!categoriesChildren.isEmpty() || categoryType.equalsIgnoreCase(title)) {
	...
	String calendarEventsURL = calendarURL.toString() + "&_8_" + "month=" + curMonth + "&_8_" + "day=" + curDay + "&_8_" + "year=" + curYear +  "&_8_"+ "selCategory=" + upcomingType + "&_8_" + "eventType="+upcomingType+","+ sb.toString();
	%>
