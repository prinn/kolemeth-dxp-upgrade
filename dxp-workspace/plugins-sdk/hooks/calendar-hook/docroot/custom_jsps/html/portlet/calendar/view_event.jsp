...
if (event.getRepeating()) {
	...
	startDate = HtmlUtil.unescape(ParamUtil.getString(request, "startDate"));
}
...
<aui:layout cssClass="event">
	<aui:column columnWidth="<%= 75 %>" cssClass="folder-column folder-column-first" first="<%= true %>">
			...
			<dd>
				<c:choose>
					<c:when test="<%= event.isTimeZoneSensitive() %>">
						...
						<%= Validator.isNotNull(startDate) ? startDate : dateFormatDate.format(Time.getDate(event.getStartDate(), timeZone)) %>
						
					</c:when>
					<c:otherwise>
					
						<%= Validator.isNotNull(startDate) ? startDate : dateFormatDate.format(Time.getDate(event.getStartDate(), TimeZoneUtil.getDefault())) %>
		...
		<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= (event != null) ? event.getEventId() : 0 %>" editable="false" label="true" name="member-price"></liferay-ui:custom-attribute>
		<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= (event != null) ? event.getEventId() : 0 %>" editable="false" label="true" name="nonmember-price"></liferay-ui:custom-attribute>
	...
	<aui:column columnWidth="<%= 25 %>" cssClass="detail-column detail-column-last" last="<%= true %>">
		...
		<c:if test="<%= !PortalUtil.isOmniadmin(themeDisplay.getUserId()) %>">			
			<liferay-util:include page="/html/portlet/calendar/view_event_action.jsp" />
		</c:if>
	</aui:column>
</aui:layout>
