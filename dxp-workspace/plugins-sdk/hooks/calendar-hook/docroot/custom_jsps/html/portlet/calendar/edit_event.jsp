...
String noEventDatestr = null;
StringBuffer nBuffer = new StringBuffer();	
if(Validator.isNotNull(event)){
	noEventDatestr=  (event.getExpandoBridge().getAttribute("no-event-date-value")).toString();
	if(Validator.isNotNull(noEventDatestr) && noEventDatestr.length() >0){
		noEventDatestr = noEventDatestr.replaceAll(",", "','");
				
		if(Validator.isNull(noEventDatestr)){
			noEventDatestr = FastDateFormatFactoryUtil.getSimpleDateFormat("MM/dd/yyyy", locale).format(Calendar.getInstance(timeZone).getTime());
		}
		
		
		nBuffer.append("'");
		nBuffer.append(noEventDatestr);
		nBuffer.append("'");
	}
}
...
<aui:form action="<%= editEventURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveEvent();" %>'>
	...
	<aui:fieldset>
		...
		<aui:input inlineLabel="left" name="timeZoneSensitive" type="hidden" value="<%= event == null ? true : event.isTimeZoneSensitive() %>" />

		<aui:input name="title" />
		
		<c:if test="<%= enableKolEmethCalendar %>">
		...
		<c:choose>
			...
			<c:when test="<%= enableKolEmethCalendar %>">
				<aui:input type="hidden" name="type" label="" value=""></aui:input>
				<style>
				.hidePermision{
					display: none;
				}
		...
		<c:if test="<%= enableKolEmethCalendar %>">
			...
			<aui:layout cssClass="aui-w80">
				<aui:column cssClass="aui-w40">
					<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= eventId %>" editable="true" label="true" name="member-price"></liferay-ui:custom-attribute>
				</aui:column >
				<aui:column cssClass="aui-w40">
					<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= eventId %>" editable="true" label="true" name="nonmember-price"></liferay-ui:custom-attribute>
				</aui:column>
			</aui:layout> 
		
			<div style="display:none;">
				<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= eventId %>" editable="true" label="true" name="reservation-deadline"></liferay-ui:custom-attribute>
			</div>
			<div style="display:none;">
				<liferay-ui:custom-attribute className="<%= CalEvent.class.getName() %>" classPK="<%= eventId %>" editable="true" label="true" name="no-event-date-value"></liferay-ui:custom-attribute>
			</div>
		</c:if>
		<c:if test="<%= !enableKolEmethCalendar %>">
	...
	<liferay-ui:panel-container cssClass="calendar-event-details" extended="<%= true %>" id="calendarEventDetailsPanelContainer" persistState="<%= true %>">
		...
		<c:choose>
			<c:when test="<%= !enableKolEmethCalendar %>">
				...
				<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="noEventDisplayDatePanel" persistState="<%= true %>" title="Don't display event on this below Date">
				
						<div class="aui-datepicker aui-helper-clearfix" id="#<portlet:namespace />expirationDatePicker">
							<aui:input size="100" name="expirationDate" type="text" label="Event Dates"  value='<%= noEventDatestr %>'>
							</aui:input>
						</div>
...
var namespace = "<portlet:namespace />";
	...
	AUI().use('aui-datepicker', function(A) {	
	       var auiDatePicker = new A.DatePicker({
	         trigger: '#<portlet:namespace />expirationDate',
	       calendar: {
	         	dates:[<%= nBuffer.toString() %> ],
	           dateFormat: '%m/%d/%Y',
	           setValue : true,
	           selectMultipleDates: true ,
	           showToday : true            
	        }	
	       }).render('#<portlet:namespace />expirationDatePicker');	      
	    });
    ...
    var selectedDates = document.<portlet:namespace />fm.<portlet:namespace />expirationDate.value ;
    if(selectedDates != "10/10/1907" ){
    	document.getElementsByName("<portlet:namespace />ExpandoAttribute--no-event-date-value--")[0].value = document.<portlet:namespace />fm.<portlet:namespace />expirationDate.value ;
	}
