...
<c:if test="<%= (messagesCount > 1) || MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.VIEW) %>">
			...
			<c:if test="<%= !hideControls && MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.ADD_DISCUSSION) %>">
				<aui:fieldset cssClass="add-comment" id='<%= randomNamespace + "messageScroll0" %>'>
					...
					<div id="<%= randomNamespace %>postReplyForm<%= i %>" style="display: none;">
						<aui:input showRequiredLabel="false" id='<%= randomNamespace + "postReplyBody" + i %>' label="comment" name='<%= "postReplyBody" + i %>' style='<%= "height: " + ModelHintsConstants.TEXTAREA_DISPLAY_HEIGHT + "px; width: " + ModelHintsConstants.TEXTAREA_DISPLAY_WIDTH + "px;" %>' type="textarea" wrap="soft" ></aui:input>
						<%
					...
					<!-- <portlet:actionURL var="createAnonymousAccountURL">
						...
						<aui:button-row>
							<%-- <aui:button id='<%= namespace + randomNamespace + "postReplyButton" + i %>' onClick='<%=createAnonymousAccountURL%>' value="<%= postReplyButtonLabel %>" /> --%>
	...
	loginURL.setParameter("struts_action", "/login/create_anonymous_account");
	%>
	...
	<aui:script>
		...
		function <%= randomNamespace %>afterLogin(emailAddress, anonymousAccount) {
			...
			document.cookie = 'saveDiscussion=true; path=/';
			if (anonymousAccount) {
		...
		function <%= randomNamespace %>deleteMessage(i) {
			...
			document.cookie = 'saveDiscussion=false; path=/';
		...
		Liferay.provide(
			...
			function(response, refreshPage) {
				...
				Liferay.after(
					...
					var workflow = <%= WorkflowDefinitionLinkLocalServiceUtil.hasWorkflowDefinitionLink(themeDisplay.getCompanyId(), scopeGroupId, MBDiscussion.class.getName()) %>;
					var cookiesVal = readCookie('saveDiscussion');
					if(cookiesVal == 'true' && workflow){
						<portlet:namespace />showStatusMessage('success', '<%= UnicodeLanguageUtil.get(pageContext, "comment-workflow-success-message") %>');						
					}else{					
