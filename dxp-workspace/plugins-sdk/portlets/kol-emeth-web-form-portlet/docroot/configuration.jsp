...
String emailAddress = WebFormUtil.getEmailToAddress(preferences);
String subject = preferences.getValue("subject", StringPool.BLANK);

String emailSubject = ParamUtil.getString(request, "emailSubject", WebFormUtil.getEmailSubject(preferences));
String emailBody = ParamUtil.getString(request, "emailBody", WebFormUtil.getEmailBody(preferences));

String adminEmailSubject = ParamUtil.getString(request, "adminEmailSubject", WebFormUtil.getAdminEmailSubject(preferences));
String adminEmailBody = ParamUtil.getString(request, "adminEmailBody", WebFormUtil.getAdminEmailBody(preferences));
	...
	<liferay-ui:panel-container extended="<%= Boolean.TRUE %>" id="webFormConfiguration" persistState="<%= true %>">
		<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="webFormGeneral" persistState="<%= true %>" title="form-information">
			<aui:fieldset>
				...
				<aui:input cssClass="lfr-input-text-container" helpMessage="Configure webcontent Id to display content in Popup dialouge box." label="Dialog Content Id" name="preferences--contentId--" value="<%= contentId %>" />
				
				<aui:input cssClass="lfr-input-text-container" helpMessage="On PopUp close redirect to page URL. <br/> Example /web/guest/home" label="Dialog Close URL" name="preferences--poUpRedirectURL--" value="<%=HtmlUtil.toInputSafe( poUpRedirectURL )%>" />
			</aui:fieldset>
		</liferay-ui:panel>
		...
		<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="webFormData" persistState="<%= true %>" title="handling-of-form-data">
			<aui:fieldset cssClass="handle-data" label="email">
				...
				<aui:input cssClass="lfr-input-text-container" name="preferences--subject--" value="<%= subject %>" type="hidden" />
					...
					<aui:field-wrapper label="body">
						...
						<div class="definition-of-terms">
							...
							<dl>
								...
								<c:if test='<%= tabs2.equalsIgnoreCase("add-admin-email") %>'>
									<dt>
										[$FROM_NAME$]
									</dt>
									<dd>
								...
								<c:if test='<%= tabs2.equalsIgnoreCase("add-client-email") %>'>
									<dt>
										[$PORTAL_URL$]
									</dt>
									<dd>
								...
								<c:if test='<%= tabs2.equalsIgnoreCase("add-admin-email") %>'>
									...
									<dd>
										<liferay-ui:message key="the-detail-of-the-client-recipient" />
									</dd>
								</c:if>
							</dl>
						</div>
